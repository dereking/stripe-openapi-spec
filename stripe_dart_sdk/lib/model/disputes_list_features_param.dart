...
class DisputesListFeaturesParam {
  /// Returns a new [DisputesListFeaturesParam] instance.
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is DisputesListFeaturesParam &&
  ...
  @override
  String toString() => 'DisputesListFeaturesParam[capturePayments=$capturePayments, destinationOnBehalfOfChargeManagement=$destinationOnBehalfOfChargeManagement, disputeManagement=$disputeManagement, refundManagement=$refundManagement]';
  ...
  /// Returns a new [DisputesListFeaturesParam] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static DisputesListFeaturesParam? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "DisputesListFeaturesParam[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "DisputesListFeaturesParam[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return DisputesListFeaturesParam(
  ...
  static List<DisputesListFeaturesParam> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <DisputesListFeaturesParam>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = DisputesListFeaturesParam.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, DisputesListFeaturesParam> mapFromJson(dynamic json) {
    final map = <String, DisputesListFeaturesParam>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = DisputesListFeaturesParam.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of DisputesListFeaturesParam-objects as value to a dart map
  static Map<String, List<DisputesListFeaturesParam>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<DisputesListFeaturesParam>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = DisputesListFeaturesParam.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
