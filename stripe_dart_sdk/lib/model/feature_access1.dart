...
class FeatureAccess1 {
  /// Returns a new [FeatureAccess1] instance.
  FeatureAccess1({
    this.cardIssuing,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is FeatureAccess1 &&
  ...
  @override
  String toString() => 'FeatureAccess1[cardIssuing=$cardIssuing, depositInsurance=$depositInsurance, financialAddresses=$financialAddresses, inboundTransfers=$inboundTransfers, intraStripeFlows=$intraStripeFlows, outboundPayments=$outboundPayments, outboundTransfers=$outboundTransfers]';
  ...
  /// Returns a new [FeatureAccess1] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static FeatureAccess1? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "FeatureAccess1[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "FeatureAccess1[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return FeatureAccess1(
  ...
  static List<FeatureAccess1> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <FeatureAccess1>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = FeatureAccess1.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, FeatureAccess1> mapFromJson(dynamic json) {
    final map = <String, FeatureAccess1>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = FeatureAccess1.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of FeatureAccess1-objects as value to a dart map
  static Map<String, List<FeatureAccess1>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<FeatureAccess1>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = FeatureAccess1.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
