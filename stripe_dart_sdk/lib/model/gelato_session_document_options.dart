...
class GelatoSessionDocumentOptions {
  /// Returns a new [GelatoSessionDocumentOptions] instance.
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is GelatoSessionDocumentOptions &&
  ...
  @override
  String toString() => 'GelatoSessionDocumentOptions[allowedTypes=$allowedTypes, requireIdNumber=$requireIdNumber, requireLiveCapture=$requireLiveCapture, requireMatchingSelfie=$requireMatchingSelfie]';
  ...
  /// Returns a new [GelatoSessionDocumentOptions] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static GelatoSessionDocumentOptions? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "GelatoSessionDocumentOptions[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "GelatoSessionDocumentOptions[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<GelatoSessionDocumentOptions> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <GelatoSessionDocumentOptions>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = GelatoSessionDocumentOptions.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, GelatoSessionDocumentOptions> mapFromJson(dynamic json) {
    final map = <String, GelatoSessionDocumentOptions>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = GelatoSessionDocumentOptions.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of GelatoSessionDocumentOptions-objects as value to a dart map
  static Map<String, List<GelatoSessionDocumentOptions>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<GelatoSessionDocumentOptions>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = GelatoSessionDocumentOptions.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
