...
class LinkedAccountOptionsCommon {
  /// Returns a new [LinkedAccountOptionsCommon] instance.
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is LinkedAccountOptionsCommon &&
  ...
  @override
  String toString() => 'LinkedAccountOptionsCommon[filters=$filters, permissions=$permissions, prefetch=$prefetch, returnUrl=$returnUrl]';
  ...
  /// Returns a new [LinkedAccountOptionsCommon] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static LinkedAccountOptionsCommon? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "LinkedAccountOptionsCommon[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "LinkedAccountOptionsCommon[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return LinkedAccountOptionsCommon(
        filters: PaymentFlowsPrivatePaymentMethodsFinancialConnectionsCommonLinkedAccountOptionsFilters.fromJson(json[r'filters']),
  ...
  static List<LinkedAccountOptionsCommon> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <LinkedAccountOptionsCommon>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = LinkedAccountOptionsCommon.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, LinkedAccountOptionsCommon> mapFromJson(dynamic json) {
    final map = <String, LinkedAccountOptionsCommon>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = LinkedAccountOptionsCommon.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of LinkedAccountOptionsCommon-objects as value to a dart map
  static Map<String, List<LinkedAccountOptionsCommon>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<LinkedAccountOptionsCommon>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = LinkedAccountOptionsCommon.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
