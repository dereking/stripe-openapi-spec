...
class ShippingAddressCollectionParams1 {
  /// Returns a new [ShippingAddressCollectionParams1] instance.
  ShippingAddressCollectionParams1({
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is ShippingAddressCollectionParams1 &&
  ...
  @override
  String toString() => 'ShippingAddressCollectionParams1[allowedCountries=$allowedCountries]';
  ...
  /// Returns a new [ShippingAddressCollectionParams1] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static ShippingAddressCollectionParams1? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "ShippingAddressCollectionParams1[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "ShippingAddressCollectionParams1[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return ShippingAddressCollectionParams1(
  ...
  static List<ShippingAddressCollectionParams1> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <ShippingAddressCollectionParams1>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = ShippingAddressCollectionParams1.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, ShippingAddressCollectionParams1> mapFromJson(dynamic json) {
    final map = <String, ShippingAddressCollectionParams1>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = ShippingAddressCollectionParams1.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of ShippingAddressCollectionParams1-objects as value to a dart map
  static Map<String, List<ShippingAddressCollectionParams1>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<ShippingAddressCollectionParams1>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = ShippingAddressCollectionParams1.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
