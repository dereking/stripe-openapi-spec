//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:openapi/api.dart';
import 'package:test/test.dart';

// tests for PhaseConfigurationParams
void main() {
  // final instance = PhaseConfigurationParams();

  group('test PhaseConfigurationParams', () {
    // List<AddInvoiceItemEntry> addInvoiceItems (default value: const [])
    test('to test the property `addInvoiceItems`', () async {
      // TODO
    });

    // num applicationFeePercent
    test('to test the property `applicationFeePercent`', () async {
      // TODO
    });

    // AutomaticTaxConfig2 automaticTax
    test('to test the property `automaticTax`', () async {
      // TODO
    });

    // String billingCycleAnchor
    test('to test the property `billingCycleAnchor`', () async {
      // TODO
    });

    // PhaseConfigurationParamsBillingThresholds billingThresholds
    test('to test the property `billingThresholds`', () async {
      // TODO
    });

    // String collectionMethod
    test('to test the property `collectionMethod`', () async {
      // TODO
    });

    // String defaultPaymentMethod
    test('to test the property `defaultPaymentMethod`', () async {
      // TODO
    });

    // InvoicesSettingsSpecsDefaultAccountTaxIds defaultTaxRates
    test('to test the property `defaultTaxRates`', () async {
      // TODO
    });

    // DocumentsParam1FilesInner description
    test('to test the property `description`', () async {
      // TODO
    });

    // SubscriptionItemCreateParamsDiscounts discounts
    test('to test the property `discounts`', () async {
      // TODO
    });

    // DurationParams duration
    test('to test the property `duration`', () async {
      // TODO
    });

    // PhaseConfigurationParamsEndDate endDate
    test('to test the property `endDate`', () async {
      // TODO
    });

    // InvoiceSettings invoiceSettings
    test('to test the property `invoiceSettings`', () async {
      // TODO
    });

    // List<ConfigurationItemParams> items (default value: const [])
    test('to test the property `items`', () async {
      // TODO
    });

    // int iterations
    test('to test the property `iterations`', () async {
      // TODO
    });

    // Map<String, String> metadata (default value: const {})
    test('to test the property `metadata`', () async {
      // TODO
    });

    // String onBehalfOf
    test('to test the property `onBehalfOf`', () async {
      // TODO
    });

    // String prorationBehavior
    test('to test the property `prorationBehavior`', () async {
      // TODO
    });

    // PhaseConfigurationParamsEndDate startDate
    test('to test the property `startDate`', () async {
      // TODO
    });

    // TransferDataSpecs2 transferData
    test('to test the property `transferData`', () async {
      // TODO
    });

    // bool trial
    test('to test the property `trial`', () async {
      // TODO
    });

    // PhaseConfigurationParamsEndDate trialEnd
    test('to test the property `trialEnd`', () async {
      // TODO
    });


  });

}
