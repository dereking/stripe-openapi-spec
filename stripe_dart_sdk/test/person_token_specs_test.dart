//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:openapi/api.dart';
import 'package:test/test.dart';

// tests for PersonTokenSpecs
void main() {
  // final instance = PersonTokenSpecs();

  group('test PersonTokenSpecs', () {
    // PersonAdditionalTosAcceptancesSpecs1 additionalTosAcceptances
    test('to test the property `additionalTosAcceptances`', () async {
      // TODO
    });

    // LegalEntityAndKycAddressSpecs address
    test('to test the property `address`', () async {
      // TODO
    });

    // JapanAddressKanaSpecs addressKana
    test('to test the property `addressKana`', () async {
      // TODO
    });

    // JapanAddressKanjiSpecs addressKanji
    test('to test the property `addressKanji`', () async {
      // TODO
    });

    // IndividualSpecsDob dob
    test('to test the property `dob`', () async {
      // TODO
    });

    // PersonDocumentsSpecs1 documents
    test('to test the property `documents`', () async {
      // TODO
    });

    // String email
    test('to test the property `email`', () async {
      // TODO
    });

    // String firstName
    test('to test the property `firstName`', () async {
      // TODO
    });

    // String firstNameKana
    test('to test the property `firstNameKana`', () async {
      // TODO
    });

    // String firstNameKanji
    test('to test the property `firstNameKanji`', () async {
      // TODO
    });

    // InvoicesSettingsSpecsDefaultAccountTaxIds fullNameAliases
    test('to test the property `fullNameAliases`', () async {
      // TODO
    });

    // String gender
    test('to test the property `gender`', () async {
      // TODO
    });

    // String idNumber
    test('to test the property `idNumber`', () async {
      // TODO
    });

    // String idNumberSecondary
    test('to test the property `idNumberSecondary`', () async {
      // TODO
    });

    // String lastName
    test('to test the property `lastName`', () async {
      // TODO
    });

    // String lastNameKana
    test('to test the property `lastNameKana`', () async {
      // TODO
    });

    // String lastNameKanji
    test('to test the property `lastNameKanji`', () async {
      // TODO
    });

    // String maidenName
    test('to test the property `maidenName`', () async {
      // TODO
    });

    // IndividualSpecsMetadata metadata
    test('to test the property `metadata`', () async {
      // TODO
    });

    // String nationality
    test('to test the property `nationality`', () async {
      // TODO
    });

    // String phone
    test('to test the property `phone`', () async {
      // TODO
    });

    // String politicalExposure
    test('to test the property `politicalExposure`', () async {
      // TODO
    });

    // AddressSpecs registeredAddress
    test('to test the property `registeredAddress`', () async {
      // TODO
    });

    // RelationshipSpecs1 relationship
    test('to test the property `relationship`', () async {
      // TODO
    });

    // String ssnLast4
    test('to test the property `ssnLast4`', () async {
      // TODO
    });

    // UsCfpbDataSpecs1 usCfpbData
    test('to test the property `usCfpbData`', () async {
      // TODO
    });

    // PersonVerificationSpecs verification
    test('to test the property `verification`', () async {
      // TODO
    });


  });

}
