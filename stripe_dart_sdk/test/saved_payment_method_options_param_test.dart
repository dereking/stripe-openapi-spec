//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:openapi/api.dart';
import 'package:test/test.dart';

// tests for SavedPaymentMethodOptionsParam
void main() {
  // final instance = SavedPaymentMethodOptionsParam();

  group('test SavedPaymentMethodOptionsParam', () {
    // List<String> allowRedisplayFilters (default value: const [])
    test('to test the property `allowRedisplayFilters`', () async {
      // TODO
    });

    // String paymentMethodRemove
    test('to test the property `paymentMethodRemove`', () async {
      // TODO
    });

    // String paymentMethodSave
    test('to test the property `paymentMethodSave`', () async {
      // TODO
    });


  });

}
