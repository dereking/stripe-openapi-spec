import 'package:test/test.dart';
import 'package:stripe_dart_sdk/stripe_dart_sdk.dart';

// tests for CustomAmountPriceDataWithProductData
void main() {
  final instance = CustomAmountPriceDataWithProductDataBuilder();
  // TODO add properties to the builder and call build()

  group(CustomAmountPriceDataWithProductData, () {
    // String currency
    test('to test the property `currency`', () async {
      // TODO
    });

    // String product
    test('to test the property `product`', () async {
      // TODO
    });

    // ProductData productData
    test('to test the property `productData`', () async {
      // TODO
    });

    // RecurringAdhoc recurring
    test('to test the property `recurring`', () async {
      // TODO
    });

    // String taxBehavior
    test('to test the property `taxBehavior`', () async {
      // TODO
    });

    // int unitAmount
    test('to test the property `unitAmount`', () async {
      // TODO
    });

    // double unitAmountDecimal
    test('to test the property `unitAmountDecimal`', () async {
      // TODO
    });

  });
}
